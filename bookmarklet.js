...
if (lscid.length > 0) {
    ...
    async function processData() {
        ...
        for (let i = 0; i < json.length; i++) {
            partnerCid = json[i].partnercompanyid;
            let locNameObj = {
    ...
    async function fillData() {
        ...
        console.log(`partnerCid = ${partnerCid}`);
            ...
            if (field.value.length <= 0) {
                switch (field) {
                    ...
                    case marketingCenterFields[1]:
                        field.value = 'https://leasestar-api.realpage.com';
                        break;
                    case marketingCenterFields[2]:
                        field.value = 'https://lead.myleasestar.com';
                        break;
                    case marketingCenterFields[3]:
                        if (g5IntDashHelperMcIntegration.vendorUserName.length > 2 && field.value.length == 0) {
                            field.value = g5IntDashHelperMcIntegration.vendorUserName;
                        }
                        break;
                    case marketingCenterFields[4]:
                        if (g5IntDashHelperMcIntegration.vendorPassword.length > 2 && field.value.length == 0) {
                            field.value = g5IntDashHelperMcIntegration.vendorPassword;
                        }
                        break;
                    /* this is for placing the partner property id */ 
                    case marketingCenterFields[5]:
                        console.log('Attempting to place partnerID');
                        if (partnerCid.length > 0 && field.value.length == 0) {
                            field.value = partnerCid;
                        }
                        break;
